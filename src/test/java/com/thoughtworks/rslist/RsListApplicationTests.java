...
class RsListApplicationTests {
    ...
    void should_update_event_given_two_fields() throws Exception {
        ...
        mockMvc.perform(patch("/rs/2").content(json).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    ...
    void should_delete_event_given_index() throws Exception {
        mockMvc.perform(delete("/rs/event/delete/1")).andExpect(status().isOk());

        mockMvc.perform(get("/rs/event/delete/list"))
                .andExpect(jsonPath("$", hasSize(2)))
                .andExpect(jsonPath("$[0].eventName", is("第二条事件")))
